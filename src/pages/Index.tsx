...
const Index = () => {
  ...
  return <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        {/* Status Chart */}
        <div className="mb-8">
          {isLoading ? <div className="h-24 animate-pulse bg-gray-100 rounded-lg"></div> : <StatusBarChart initiatives={filteredInitiatives} categoryId={categoryFilter !== 'all' ? categoryFilter : undefined} statusMap={statusMap} categoryMap={categoryMap} showPercentages={true} />}
        </div>
        
        {/* Filter and Search */}
        <div className="mb-8">
          <FilterSearchBar searchQuery={searchQuery} setSearchQuery={setSearchQuery} statusFilter={statusFilter} setStatusFilter={setStatusFilter} statusMap={statusMap} hasFilters={hasFilters} clearFilters={clearFilters} />
        </div>
        ...
        <div className="mb-8">
          <Tabs defaultValue="themes" value={activeTab} onValueChange={value => setActiveTab(value as 'themes' | 'list')}>
            ...
            <TabsContent value="themes" className="space-y-8">
              <ThemeBasedOverview initiatives={filteredInitiatives} selectedCategory={categoryFilter} onCategorySelect={setCategoryFilter} isLoading={isLoading} categoryMap={categoryMap} statusMap={statusMap} />
              
              {categoryFilter !== 'all' && <div className="mt-8">
                  <h2 className="text-xl font-semibold mb-4">
                    {categoryMap[categoryFilter]?.label} ({filteredInitiatives.length} Vorhaben)
                  </h2>
                  <InitiativesList initiatives={filteredInitiatives} isLoading={isLoading} categoryMap={categoryMap} statusMap={statusMap} />
                </div>}
            ...
            <TabsContent value="list" className="space-y-8">
              <div>
                <h2 className="text-xl font-semibold mb-4">
                  {categoryFilter !== 'all' ? `${categoryMap[categoryFilter]?.label} (${filteredInitiatives.length} Vorhaben)` : `Alle Vorhaben (${filteredInitiatives.length})`}
                </h2>
