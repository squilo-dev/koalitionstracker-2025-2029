...
const StatusBarChart: React.FC<StatusBarChartProps> = ({
  ...
  const statusOrder = ['umgesetzt', 'teilweise-umgesetzt', 'begonnen', 'nicht-begonnen', 'verschoben'].filter(id => statusIds.includes(id));
      ...
      <CardContent>
        <div className="space-y-4">
          <HoverCard>
            <HoverCardTrigger asChild>
              <div className="h-6 w-full bg-gray-100 rounded-full overflow-hidden flex">
                ...
                return percentage > 0 ? <div key={statusId} className="h-full transition-all duration-500" style={{
                  width: `${percentage}%`,
            ...
            <HoverCardContent className="w-auto p-2">
              <div className="text-xs space-y-1">
                ...
                const percentage = statusPercentages[statusId] || 0;
                const status = statusMap[statusId];
                return percentage > 0 ? <div key={statusId} className="flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            const status = statusMap[statusId];
            return count > 0 ? <div key={statusId} className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-sm" style={{
                backgroundColor: status?.color || '#cbd5e1'
              }}></div>
                  <span className="text-sm">
                    {status?.label} {showPercentages ? `(${percentage.toFixed(1)}%)` : `(${count})`}
                  </span>
                </div> : null;
          })}
          </div>
