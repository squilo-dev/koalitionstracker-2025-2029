...
import { BookOpen, Briefcase, Building, Cloud, Database, Globe, Heart, Shield, Truck, Wallet, FileText } from 'lucide-react';
...
const ThemeOverviewCard: React.FC<ThemeOverviewCardProps> = ({
  ...
  return <Card className={cn("cursor-pointer transition-all duration-200 hover:shadow-md", isActive ? "ring-2 ring-coalition-primary" : "")} onClick={onClick}>
          ...
          <HoverCard>
            <HoverCardTrigger asChild>
              <div className="h-6 w-full bg-gray-100 rounded-full overflow-hidden flex">
                ...
                return percentage > 0 ? <div key={status} className={cn(getStatusColor(status), "h-full transition-all duration-500")} style={{
                  width: `${percentage}%`
                }}></div> : null;
              })}
              </div>
            ...
            <HoverCardContent className="w-auto p-2">
              <div className="text-xs space-y-1">
                ...
                const percentage = statusPercentages[status];
                return percentage > 0 ? <div key={status} className="flex items-center gap-1.5">
